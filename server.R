...
server <- function(input, output,session) {
  ...
  observeEvent(input$newchallenge, {
      ...
      updateCheckboxInput(session = session, inputId ="yourownline",
                          label = "Create your own line (Red)", value = FALSE)
      updateCheckboxInput(session = session, inputId = "correlation",
                          label = "Show correlation value", value = FALSE)
      updateCheckboxInput(session = session, inputId = "regressionline",
                          label = "Show regression line (black)", value = FALSE)
  })
  # Reset Function
  ...
  output$question <- renderText({
    if (c$right == 1){
      ...
      as their regression (then check the “Show regression” 
      ...
      (then check the “Show regression” box to see how you did)."   
    }
    else if  (c$right == 5){
